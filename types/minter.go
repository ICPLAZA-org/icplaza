...
var (
	...
	gTotalsupply = sdk.TokensFromConsensusPower(int64(700000000), ethermint.PowerReduction)

	// inflation of a new block
	// 3 * (60 * 60 * 8760 / 6) = sdk.NewDec(15768000000000)
	// g_inflationPerBlock = sdk.NewInt(1200000)
	gInflationperblock = sdk.TokensFromConsensusPower(12, ethermint.PowerReduction.QuoRaw(10))
...
func (h MintHook) BeforeNextAnnualProvisions(blockHeight int64, blocksPerYear uint64, totalSupply sdk.Int, customProvision bool) sdk.Dec {
	...
	if totalSupply.GT(gTotalsupply) {
		rc = sdk.ZeroDec()
		...
		rc = sdk.NewDecFromInt(gInflationperblock).MulInt(sdk.NewInt(int64(blocksPerYear)))
	}
